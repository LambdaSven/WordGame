...
let words_table_definition = "CREATE TABLE IF NOT EXISTS words (word TEXT NOT NULL, lobby TEXT NOT NULL, user INT NOT NULL, PRIMARY KEY(word, lobby))"
...
let add_word word lobby user = 
    let statement = prepare database "INSERT INTO words VALUES (?, ?, ?)" in
    let _ = bind_text statement 1 word
    and _ = bind_text statement 2 lobby
    and _ = bind_text statement 3 user in
